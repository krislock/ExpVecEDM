%%*************************************************************************
%% For estimation without anchors, the estimated positions differ from the 
%% true positions up to translation, rotation, or reflection.  
%% Thus to evaluate the goodness of the estimated positions, an affine
%% mapping must be performed on the estimated positions to match the
%% original positions.
%%
%% [Xmatch,errtrue,Map] = matchposition(Xopt,PP,tvar);
%%
%% min_A norm(A*(Xopt-Xcenter) - (PP-Pcenter),'fro')
%%
%% Xmatch = PPcenter + A*(Xopt-Xoptcenter)
%%
%%*************************************************************************

  function [Xmatch,errtrue,Map] = matchposition(Xopt,PP,tvar,map_options);

    if (nargin < 4); map_options = 2; end

    tvartol = mean(tvar(tvar < 0.01)); 
    [tvarsort,idx] = sort(tvar);
    mid = max(find(tvarsort < 0.5*tvartol)); 
    lentmp = min([500,floor(length(idx)/2),max(50,mid)]);
  
    X2 = PP(:,idx(1:lentmp));
    X1 = Xopt(:,idx(1:lentmp));
    if (map_options == 1)
       [A,X1center,X2center] = findaffinemap(X1,X2);
       ee = ones(1,size(Xopt,2));
       Xmatch = A*(Xopt-X1center*ee) + X2center*ee;
    else   
      [dummy,dummy2,Map] = procrustes(X2',X1');
      Xmatch = (Map.b*Map.T')*Xopt + diag(Map.c(1,:))*ones(size(Xopt));
    end  
    tmp = PP - Xmatch;
    errtrue = sqrt(sum(tmp.*tmp));
%%*************************************************************************
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
